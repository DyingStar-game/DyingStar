...
static func _get_base_stalk_mesh() -> Mesh:
	...
	if _base_stalk_mesh == null:
		push_warning("[GrasslandSpawner] Could not load grass stalk mesh: %s" % GRASS_STALK_PATH)
	return _base_stalk_mesh
...
static func _get_grass_material() -> Material:
	if _grass_mat:
		return _grass_mat
	var shader := load("res://assets/materials/planet/grassland_grass.gdshader") as Shader
	if shader:
		_grass_mat = ShaderMaterial.new()
		_grass_mat.shader = shader
	else:
		# Fallback: simple green material if shader is missing.
		push_warning("[GrasslandSpawner] grassland_grass.gdshader not found — using fallback")
...
static func _build_tuft_mesh(blade_count: int = BLADES_PER_TUFT) -> ArrayMesh:
	...
	if base == null or base.get_surface_count() == 0:
		push_warning("[GrasslandSpawner] Base stalk mesh unavailable — using procedural fallback")

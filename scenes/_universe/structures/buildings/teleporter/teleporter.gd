...
## Key in [constant Globals.DISABLED_DEV_TOOLS]. It has no InputMap binding — the cabin is walked
...
func _ready() -> void:
	...
	_ui.set_enabled(not Globals.is_dev_tool_disabled(DEV_TOOL))
...
func update_screen(data: Dictionary) -> void:
	...
	if Globals.is_dev_tool_disabled(DEV_TOOL):
		push_warning("[Teleporter] refused: the teleporter is switched off (Globals.DISABLED_DEV_TOOLS)")
		return

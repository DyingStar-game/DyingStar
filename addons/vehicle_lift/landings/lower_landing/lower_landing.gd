...
@onready var landing_interface: Control = %"Lower Landing Interface"
...
func _ready() -> void:
	...
	if Engine.is_editor_hint():
		...
		var railing_json_path: StringName = &"res://addons/vehicle_lift/landings/lower_landing/railing_datas.json"
...
func _on_call_button_triggered(datas: Dictionary) -> void:
	if GameOrchestrator.is_server():
		return
	print_rich("[color=green]On a cliqué sur CALL et on est dans lower_landing.gd[/color]")
